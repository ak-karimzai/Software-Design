...
void AuthRepositoryImplPg::addNewUser(const std::string &login, const std::string &password, const int teacherId)
{
    try {
        ...
        std::string query = AuthQueryGenerator::getNewUserQuery(login,
                                                                password,
                                                                teacherId);
...
void AuthRepositoryImplPg::deleteUserDetails(const int teacherId)
{
    Auth auth;
    try {
        if (!connection->is_open()) {
        ...
        std::string query = AuthQueryGenerator::getDeleteDetailsQuery(teacherId);
        pqxx::work work(*connection);
        pqxx::result result = work.exec(query);
        work.commit();
    }  catch (const std::exception & e) {
